...
describe("Can send an api request to real free api", () => {
    test("Can simply call an api and get todays weather conditions", async () => {
        expect((await forecast("").fetch(global.fetch).at(new Date()).is())?.icon).toBeTruthy();
    });
    test("Can call an api and get information on past date events", async () => {
        ...
        const past = forecast(apiKey).fetch(global.fetch);
        ...
        expect((await past.fetch(global.fetch).at(new Date(2020, 6, 19, 0, 0, 0)).rainy())?.description).toBe('shower rain');
    });
    test("Can call api to find out how much it rains outside", async () => {
        expect(await forecast(apiKey).fetch(global.fetch).in('Prague').at(new Date()).rain()).not.toBeNull();
    });
});